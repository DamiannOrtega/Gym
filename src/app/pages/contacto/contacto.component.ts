...
export class ContactoComponent implements OnInit {
  ...
  onMedioChange(event: Event, medio: string): void {
    ...
    if (this.mediosSeleccionados.includes('WhatsApp')) {
      ...
      telefonoControl?.setValidators(null); // Elimina los validadores cuando no es WhatsApp
    }
    telefonoControl?.updateValueAndValidity({ onlySelf: true });
  ...
  guardar(): void {
    ...
    const nuevoContacto = {
      ...
      mediosContacto: this.mediosSeleccionados, // Guardamos los medios de contacto seleccionados
    };

    // Mostrar los datos para depuración
    console.log('Guardando contacto:', nuevoContacto);

    // Llamar al servicio FirebaseService para guardar los datos en Firestore
    this.firebaseService.agregarDato('contactos', nuevoContacto)
      .then(() => {
        // Mostrar mensaje de éxito al usuario usando SweetAlert
      ...
      .catch((error) => {
        ...
        Swal.fire('Error', 'Hubo un problema al guardar tu mensaje.', 'error');
      });
  }//guardar
